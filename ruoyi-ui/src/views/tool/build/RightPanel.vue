    ...
    <div class="field-box">
      ...
      <el-scrollbar class="right-scrollbar">
        ...
        <el-form v-show="currentTab==='field' && showField" size="small" label-width="90px">
          ...
          <el-form-item
                    v-if="activeData['icon']!==undefined && activeData.__config__.tag === 'el-button'" label="按钮图标">
          ...
          <el-form-item
                  v-if="activeData.type !== undefined && 'el-date-picker' === activeData.__config__.tag" label="时间类型">
          ...
          <el-form-item
                v-if="activeData.type !== undefined && activeData.__config__.tag === 'el-button'" label="按钮类型">
          ...
          <template v-if="['el-checkbox-group', 'el-radio-group', 'el-select'].indexOf(activeData.__config__.tag) > -1">
            ...
            <draggable :list="activeData.options" :animation="340" group="selectItem" handle=".option-drag">
              <el-tooltip placement="top">
                <div slot="content">{{selectInterfRespFormat}}</div>
                <el-form-item label="接口地址" v-if="activeData.__config__.tag === 'el-select'">
                    <el-input v-model="activeData.__config__.url" placeholder="请输入接口地址"
                              clearable @blur="$emit('fetch-data', activeData)" />
                </el-form-item>
              </el-tooltip>
              <div v-if="!activeData.remote" v-for="(item, index) in (activeData.__config__.tag === 'el-select' ? activeData.options : activeData.__slot__.options)" :key="index" class="select-item">
            ...
            <div style="margin-left: 20px;" v-if="!activeData.remote">
          ...
          <el-form-item v-if="activeData.__config__.tag === 'el-select'" label="是否远程">
            <el-switch v-model="activeData.remote" @change="remoteChange"/>
          </el-form-item>
...
export default {
  ...
  data() {
    return {
      currentTab: 'field',
      currentNode: null,
      selectInterfRespFormat: '返回数据结构：{code: 200, msg: "ok", data: [{value: "1", label: "选项一"}, {value: "2", label: "选项二"}]}',
      dialogVisible: false,
      iconsVisible: false,
  ...
  methods: {
    ...
    remoteChange(val) {
      console.log(val)
      this.$set(this.activeData, 'filterable', val)
    },
    rangeChange(val) {
      this.$set(
        this.activeData.__config__,
